...
# Requirements:
 - **Godot 4.6.3+** (Forward Plus renderer)
...
# Migration Notes (4.3 → 4.6.3):
 - Updated project target from Godot 4.3 to Godot 4.6.3.
 - Replaced deprecated `AnimationPlayer.play_backwards()` calls with `play()` using a negative `custom_speed` parameter.
 - Updated SphynxMotionBlurToolkit addon version tag to 4.6.3 (CompositorEffect API unchanged).
 ...
 - After opening in Godot 4.6.3, allow the editor to re-import assets and re-save `.tscn`/`.tres` files when prompted.
